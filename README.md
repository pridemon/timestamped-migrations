...
You can add or remove a foreign key using the following functions:
	...
	function add_foreign_key($table_name, $index_name, $column_name, $referenced_table_name, $referenced_column_name, $on_update = 'NO ACTION', $on_delete = 'NO ACTION')
	function remove_foreign_key($table_name, $index_name)

Foreign key action: ``RESTRICT``, ``CASCADE``, ``SET NULL`` and ``NO ACTION`` allowed

Example:

	$this->add_foreign_key('customer', 'fk_customer_from_city', 'city_id', 'city', 'id', 'CASCADE', 'SET NULL');
	$this->remove_foreign_key('customer', 'fk_customer_from_city');
